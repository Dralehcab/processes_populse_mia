...
class ArrayToNifti(ProcessMIA):
    ...
    def list_outputs(self):
        ...
        if not self.nifti_jason_header_in:
            ...
            out_filename = os.path.splitext(out_filename)[0] + '.nii'
            output_dict = {'file_image_out': os.path.join(path, out_filename)}

        # Generating the inheritance dictionary
        inheritance_dict = {output_dict['file_image_out']: self.nifti_jason_header_in}
        print('Array To Nifti ...')
        print(output_dict, inheritance_dict)
    ...
    def _run_process(self):
        if not self.nifti_jason_header_in:
            ...
            nib.save(nifti_image,os.path.join(path, out_filename))

            print('Saving ...')
            print(nifti_image,os.path.join(path, out_filename))
            print('Jason Added ...')
...
class NiftiToArray(ProcessMIA):
    ...
    def _run_process(self):
        ...
        if not self.file_image_in:
            ...
            print('Saving Jason ...')
            print(os.path.join(path, out_filename))
